...
const LockedItem = ({
  ...
  return (
    <div className={`${className } bg-white w-1/2 max-w-3xl flex items-center flex-col rounded-lg`}>
        ...
        <div className='flex flex-col blur w-full p-5 h-60 justify-center  px-16'>
        ...
        <Link to='/login' className='flex items-center flex-col text-center absolute p-5 h-52 justify-center  mt-20 '>
           <p className='text-gray-700 text-sm font-bold w-60'> جهت مشاهده {title} خود وارد حساب کاربری شوید.</p>
        </Link>
    </div>
  )
}
